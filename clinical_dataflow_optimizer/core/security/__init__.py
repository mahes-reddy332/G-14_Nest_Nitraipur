...
class InputValidator:
    ...
    @classmethod
    def check_prompt_injection(cls, value: str, field_name: str) -> str:
        """Check for prompt injection patterns"""
        for pattern in cls.PROMPT_INJECTION_PATTERNS:
            if re.search(pattern, value, re.IGNORECASE):
                logger.warning(f"Potential prompt injection attempt in {field_name}")
                raise ValidationError(
                    field_name,
                    "Input contains potentially unsafe prompt injection patterns",
                    value
                )
        return value
    ...
    def validate_query(cls, value: str, field_name: str = "query") -> str:
        ...
        cls.check_sql_injection(value, field_name)
        cls.check_prompt_injection(value, field_name)
        return value
...
class DataEncryption:
    ...
    def __init__(self, key: bytes = None):
        """Initialize with a 32-byte key (AES-256)"""
